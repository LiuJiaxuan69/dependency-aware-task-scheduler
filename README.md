...
该项目作为实验项目，并不适用于生产环境，但该项目的性能是有保障的，并且代码结构清晰，易于理解和扩展。
该项目完全由系统底层的信号量和线程库实现，之所以这样完全是为了满足实验对“多线程+有意义的同步问题+使用 OS 底层信号量和 PV 操作”的要求。若希望在这个基础上进行生产级开发，建议替换为 C++ 标准库的同步原语（`std::mutex`、`std::condition_variable`）以及更成熟的线程池实现。
...
硬件/系统信息
- CPU: Intel(R) Xeon(R) Platinum 8255C @ 2.50GHz（2 vCPU，1 线程/核）
- 内存: 1.8 GiB（可用约 0.3 GiB，存在 4 GiB swap）
- 内核/OS: Linux 6.1.0-39-amd64 x86_64 (Debian 12)

基准设置
- 编译: Release，C++17
- 线程池规模: 默认模板参数（PoolSize=24）
- 负载: 每个子任务进行固定 CPU 迭代 `work=5000`
- 模式: indep（无依赖）、chain（线性依赖）、barrier（三阶段屏障）

结果（单位：throughput 任务/秒；tasks=N_mains×N_subs）

| 模式   | 主任务×子任务 | 总任务数 | submit_ms | total_ms | 吞吐（tasks/s） |
|--------|----------------|----------|-----------|----------|-----------------|
| indep  | 50×20          | 1,000    | 7         | 10       | 100,000         |
| chain  | 50×20          | 1,000    | 11        | 14       | 71,428          |
| indep  | 100×50         | 5,000    | 51        | 61       | 81,967          |
| barrier| 100×50         | 5,000    | 67        | 78       | 64,103          |
| chain  | 100×50         | 5,000    | 85        | 91       | 54,945          |
| indep  | 200×100        | 20,000   | 260       | 275      | 72,727          |
| chain  | 200×100        | 20,000   | 337       | 355      | 56,338          |

观察与结论
- 峰值吞吐出现在 indep 50×20 下，约 100k tasks/s（在 2 vCPU 环境；更大任务规模时吞吐受提交/调度开销与资源竞争影响略降）。
- indep > barrier > chain：依赖越强，吞吐越低，主要受依赖等待与广播开销影响。
- 增大 mains×subs 可提高并行度，但会引入更多调度与同步开销，存在折中点；在该机器上 1k~5k 任务量区间表现较优。

如何复现
```bash
cd build
./benchmarks/bench --mains 50 --subs 20 --work 5000 --pattern indep
./benchmarks/bench --mains 100 --subs 50 --work 5000 --pattern chain
./benchmarks/bench --mains 100 --subs 50 --work 5000 --pattern barrier
```
